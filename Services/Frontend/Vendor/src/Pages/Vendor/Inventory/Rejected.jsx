...
import { Search, Filter, Package, AlertCircle, Eye, Edit3, XCircle, AlertTriangle, Ban } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const rejectedProducts = res.data.filter((product) => product.status?.toLowerCase() === "rejected");
        ...
        <div className="mb-8">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 mb-2">Rejected Products</h1>
                <p className="text-gray-600">Products that were rejected and need your attention</p>
              </div>
              <div className="mt-4 lg:mt-0">
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                  <AlertTriangle className="h-4 w-4" />
                  <span>{filteredProducts.length} Rejected Products</span>
                </div>
              </div>
            </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600 font-medium">Loading rejected products...</p>
            </div>
          </div>
        ) : error ? (
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No Rejected Products</h3>
            <p className="text-gray-600">
              {searchQuery 
                ? "No rejected products match your search criteria." 
                : "You don't have any rejected products at the moment."}
            </p>
          </div>
        ) : (
