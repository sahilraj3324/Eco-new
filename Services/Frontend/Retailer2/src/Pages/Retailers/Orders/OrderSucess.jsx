...
const OrderSuccessPage = () => {
  ...
  // Extract payment info from Cashfree payment details
  const cashfreePaymentId = paymentDetails?.paymentId || paymentDetails?.cf_payment_id || paymentId;
  const actualPaymentMethod = paymentDetails?.paymentMethod || paymentMethod || 'Cashfree';
  ...
  const clearOrderedCartItems = async () => {
    ...
    try {
      ...
      if (!cartItemIds || cartItemIds.length === 0) {
        console.log('No cart items to clear - order created directly');
        return;
      }
  ...
  useEffect(() => {
    const updateAllStocks = async () => {
      if (!orderItemsForStock || orderItemsForStock.length === 0) {
        console.log('No stock items to update - skipping stock update');
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="max-w-2xl w-full bg-white/80 backdrop-blur-lg rounded-3xl shadow-2xl border border-white/20 overflow-hidden relative">
        ...
        <div className="p-8">
          ...
          {(cashfreePaymentId || paymentId || isPaymentSuccess || paymentDetails) && (
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl shadow-lg border border-blue-200 p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white/70 rounded-xl p-4 border border-blue-100">
                  ...
                  <p className="text-lg font-mono font-bold text-blue-600 break-all">
                    {cashfreePaymentId || paymentId || 'CASHFREE_SUCCESS'}
                  </p>
                  {isManualVerification && (
                    <p className="text-xs text-orange-600 mt-1">⚠️ Pending verification</p>
                  )}
                  {paymentDetails && (
                ...
                <div className="bg-white/70 rounded-xl p-4 border border-blue-100">
                  ...
                  <p className="text-xs text-green-600 mt-1">✅ Secured by Cashfree</p>
                </div>
              </div>
                ...
                <div className="mt-4 bg-white/50 rounded-xl p-4 border border-blue-100">
                  ...
                  <div className="text-xs text-gray-600 space-y-1">
                    {paymentDetails.paymentMessage && (
                      <p><span className="font-medium">Status:</span> {paymentDetails.paymentMessage}</p>
                    )}
                    <p><span className="font-medium">Gateway:</span> Cashfree Payments</p>
                    <p><span className="font-medium">Transaction Time:</span> {new Date().toLocaleString()}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="bg-gradient-to-br from-gray-50 to-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-8">
            ...
            {isBulkOrder ? (
              <>
                ...
                {/* Order IDs for Bulk Order */}
                {orderIds && orderIds.length > 0 && (
                  <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-4 border border-indigo-100">
                    <div className="flex items-center gap-2 mb-3">
                      <Shield className="h-4 w-4 text-indigo-600" />
                      <span className="text-sm font-medium text-indigo-700">Order IDs</span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {orderIds.map((orderId, index) => (
                        <div key={index} className="bg-white/70 rounded-lg p-3 border border-indigo-100">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500 mt-3">
                ✨ {!cartItemIds || cartItemIds.length === 0
                  ? 'This was a direct purchase - your cart remains unchanged.'
                  : isFullCartCheckout 
